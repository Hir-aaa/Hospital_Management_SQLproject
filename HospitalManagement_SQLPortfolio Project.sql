...
###Rooms Data###
insert into Rooms (room_id,room_number,department_id,capacity)
VALUES
(1, '101A', 3, 2),
(2, '101B', 3, 1),
(3, '102', 5, 2),
...
#PART 2#--------------------------------------------------------------------
#For all the emergency appointments, show patient name, date of birth as well as age-group: #
...
select
    ...
    a.appointment_date,
    CASE 
        WHEN (YEAR(a.appointment_date) - YEAR(p.dob)) <= 18 THEN 'Pediatric'
        WHEN (YEAR(a.appointment_date) - YEAR(p.dob)) BETWEEN 19 AND 64 THEN 'Adult'
        ELSE 'Geriatric'
    END AS age_group
from Appointments a
JOIN Patients p ON a.patient_id = p.patient_id
...
With Appointments_List As(
 select
 p.name,
 a.appointment_date,
 a.reason,
Row_Number() Over (Partition by p.patient_id
Order by a.appointment_date DESC) As rank_number
From Patients p
JOIN Appointments a ON p.patient_id = a.patient_id
)
 Select name,appointment_date,reason
...
Select
  ...
  Dense_rank() Over (Order By Count(p.prescription_id) DESC) As med_rank
From Medications m
...
With Hospital_Stats As (
  Select
   ...
   Dense_rank() Over (Order by COUNT(d.doctor_id) ASC)
   AS low_rank
From Hospitals h
JOIN doctors d ON h.hospital_id = d.hospital_id
Group by h.name)
...
With DeptCapacities AS (
    select 
        ...
        DENSE_RANK() OVER(PARTITION BY h.hospital_id ORDER BY SUM(r.capacity) DESC) as rnk
    from hospitals h
    JOIN departments dept ON h.hospital_id = dept.hospital_id
    JOIN rooms r ON dept.department_id = r.department_id
    Group by h.hospital_id, h.name, dept.department_id, dept.name
)
...
###--- Find tableau data attached or here: https://public.tableau.com/views/HospitalManagement_17750413382830/Dashboard1?:language=en-US&publish=yes&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link ----###

###--Average room capacity per department or per hospital--###

Select 
h.name, AVG(r.capacity) as avg_capacity
From hospitals h
